...
public class Boss5 : MonoBehaviour
{
    ...
    public void SpawnFireColumn()
    {
        if (fireColumnCoroutine == null)
        {
            Debug.Log("Boss6: Bắt đầu spawn Fire Column!");
    ...
    private IEnumerator SpawnFireColumnsRoutine()
    {
        ...
        // Độ trễ 1 giây trước khi bắt đầu spawn cột lửa đầu tiên
        yield return new WaitForSeconds(1f);
        while (fireColumnsSpawned < maxFireColumns)
        {
            SpawnSingleFireColumn();
            StartCoroutine(ShakeCameraSmooth());
            Debug.Log($"Boss6: Spawned Fire Column {fireColumnsSpawned + 1}/{maxFireColumns}");
            fireColumnsSpawned++;
            yield return new WaitForSeconds(fireColumnInterval);
        }
        // Cooldown bắt đầu khi cột lửa cuối cùng spawn ra
        lastSkill1Time = Time.time;
        fireColumnCoroutine = null;
        Debug.Log("Boss6: Đã spawn xong tất cả Fire Column!");
        PlayIdleAnimation();
        isUsingSkill1 = false;
        isAttacking = false;
        isMoving = false;
