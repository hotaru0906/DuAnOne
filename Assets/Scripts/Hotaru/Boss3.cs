...
public class Boss3 : MonoBehaviour
{
    ...
    public Transform bulletSpawnPoint; // Điểm spawn viên đạn (có thể là con của Boss3)
    public float bulletSpeed = 5f;
    public float bulletLifetime = 3f;
    public float bulletDamage = 20f;
    
    [Header("Skill 1 Settings (Teleport)")]
    public float skill1Cooldown = 10f; // Skill 1 mỗi 10 giây
    public float teleportDistance = 2f; // Khoảng cách teleport đằng sau player
    public string teleportStartAnimationName = "Boss3_TP1"; // Animation khi bắt đầu teleport
    public string teleportEndAnimationName = "Boss3_TP2"; // Animation khi kết thúc teleport
    ...
    public string hitAnimationName = "Boss3_Hit";
    ...
    void Start()
    {
        ...
        Debug.Log($"Boss3 initialized with {currentHealth}/{maxHealth} health");
    }
    ...
    public void TeleportBehindPlayer()
    {
        if (player != null)
        {
            // Tính vị trí đằng sau player
            Vector3 teleportPosition = CalculateBehindPlayerPosition();
            
            // Teleport boss đến vị trí mới
            transform.position = teleportPosition;
            
            // Flip hướng về player sau khi teleport
            float directionX = player.position.x - transform.position.x;
            FlipSprite(directionX);
            
            // Chạy animation teleport end (Boss3_TP2)
            PlayTeleportEndAnimation();
            
            Debug.Log($"Boss3 teleported behind player to position: {teleportPosition}");
        }
        else
        {
            Debug.LogWarning("Cannot teleport: Player is null");
            EndSkill1(); // Kết thúc skill nếu không thể teleport
        }
    }
    
    // Tính toán vị trí đằng sau player
    Vector3 CalculateBehindPlayerPosition()
    {
        Vector3 playerPosition = player.position;
        
        // Xác định hướng mà player đang face (giả sử player có thuộc tính facingRight hoặc localScale)
        // Nếu không có thông tin này, ta sẽ dựa vào vị trí hiện tại của boss so với player
        float directionX = transform.position.x - playerPosition.x;
        
        // Nếu boss đang ở bên trái player, teleport về bên phải player (và ngược lại)
        float teleportX;
        if (directionX < 0)
        {
            // Boss ở bên trái, teleport về bên phải
            teleportX = playerPosition.x + teleportDistance;
        }
        else
        {
            // Boss ở bên phải, teleport về bên trái
            teleportX = playerPosition.x - teleportDistance;
        }
        
        // Giữ nguyên Y và Z
        return new Vector3(teleportX, playerPosition.y, playerPosition.z);
    }
    ...
    {
        isUsingSkill1 = false;
        Debug.Log("Boss3 Teleport skill ended, returning to normal state");
    ...
    void AttackPlayer()
    {
        Debug.Log("Boss3 attacked Player! Player took damage!");
        if (hitbox != null)
        {
    ...
    void CreateBullet()
    {
        ...
        {
            // Xác định vị trí spawn viên đạn
            Vector3 spawnPosition = bulletSpawnPoint != null ? bulletSpawnPoint.position : transform.position;
            
            // Tạo viên đạn
            GameObject bullet = Instantiate(bulletPrefab, spawnPosition, Quaternion.identity);
            
            // Xác định hướng bắn dựa trên hướng boss đang face (chỉ trục X)
            Vector2 shootDirection = facingRight ? Vector2.right : Vector2.left;
            
            // Thiết lập thuộc tính và hướng viên đạn bằng SendMessage
            bullet.SendMessage("SetBulletProperties", new object[] { bulletSpeed, bulletLifetime, bulletDamage }, SendMessageOptions.DontRequireReceiver);
            bullet.SendMessage("SetDirection", shootDirection, SendMessageOptions.DontRequireReceiver);
            
            Debug.Log($"Boss3 created bullet shooting {(facingRight ? "right" : "left")}");
        }
        else
        {
    ...
    void PlayTeleportStartAnimation()
    {
        if (animator != null)
        {
            animator.Play(teleportStartAnimationName);
